	 ...
	 public void addProwadzacy(Prowadzacy prowadzacy) {
	   ...
	   public List<Prowadzacy> listProwadzacy() {
		...
		public void aktywacja(String Login, boolean Aktywowany) {
			 sessionFactory.getCurrentSession().createQuery("UPDATE Prowadzacy SET Aktywowany=:Aktywowany WHERE Login=:Login").setString("Login", Login).setBoolean("Aktywowany", Aktywowany).executeUpdate();
		}
